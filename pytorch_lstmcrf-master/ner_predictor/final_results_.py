    ...
    if 'STRUCTURE' in dict_:
        ...
        for key in structure_:
            ...
            for section in sections:
                for sen in section:
                    structure_count[key] += sen.count(key)
            ...
            if prediction[i][0] == 'B':
                ...
                if label in ['CODE', 'STRUCTURE'] or (label in ['METHOD','PARAMETER'] and ( filter_maethodparam is False or inst.input.heading in valid_sections)) or (label in ['MATERIAL'] and (filter_material is False or inst.input.heading in ['TITLE', 'ABSTRACT'] or int(inst.input.heading) <= 0)):
                    ...
                    if label == 'PARAMETER':
                        ...
                        values_p[highlighted.strip()].extend(find_values(inst, 'PARAMETER', highlighted.strip()))
...
def print_results_to_file(file, results, matched_dict, show_unmatched, values_s, outputs_s):
    ...
    # results = matched_dict if show_unmatched else dict_overlap
...
def predict(sentences: Union[str, List[str]], path, insts, filter_maethodparam, filter_material, filter_structure, show_unmatched):
    ...
    # print_results_to_file('results/'+''.join((''.join(path.split('/')[-1])).split('.')[:-1])+'.predicted.categorised.txt', predictions, matched_pred_dict, show_unmatched, values_s, outputs_s)
    # print_results_to_file('results/'+''.join((''.join(path.split('/')[-1])).split('.')[:-1])+'.true.categorised.txt', true_labels, matched_true_dict, show_unmatched, {}, {})

    return dict_overlap, predictions, values_s, values_p, true_labels
...
if __name__ == '__main__':
    ...
    for subdir, dirs, files in os.walk(paper_dir):
        for file in tqdm(files, desc="Files"):
            ...
            dict_overlap, predictions,_,_,_ = predict(sentences, paper, insts, filter_maethodparam, filter_material, filter_structure, show_unmatched)
